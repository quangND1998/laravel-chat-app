<template>
  ...
  >
    ...
    <MessageItem
      v-for="(message, index) in privateChat.message.list"
      :key="index"
      :message="message"
      @showEmoji="showEmoji"
    />
...
export default {
  ...
  mounted() {
    ...
    $("#private_room").on("scroll", async () => {
      ...
      if (
        scroll < 1 &&
        this.privateChat.message.currentPage < this.privateChat.message.lastPage
      ) {
        this.$emit(
          "getMessages",
          this.privateChat.roomId,
          this.privateChat.message.currentPage + 1,
          true
        );
      }
    });
  ...
  methods: {
    saveMessage() {
      this.$emit(
        "saveMessage",
        this.inputMessage,
        this.privateChat.selectedReceiver.id
      );
      this.inputMessage = "";
    },
