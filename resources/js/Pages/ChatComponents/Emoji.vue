<template>
    <transition name="slide-fade">
      <div
        class="emoji-container flex items-center"
        :style="{left: `${emojiCoordinates.x}px`, top: `${emojiCoordinates.y - 60}px`}"
        v-if="isShow"
      >
        <div
          class="emoji-item"
          v-for="e in $page.props.emojis"
          :key="e.src"
          @click="selectEmoji(e)"
        >
  ...
  export default {
    ...
    mounted () {
      $(document).on('click', (e) => {
        ...
        if ($(e.target).attr('class') === 'fa-regular fa-face-grin') {
            console.log('aaaa',this.isShow)
          return
        }
        // check if the clicked area is dropDown or not
        if (this.isShow && container.has(e.target).length === 0) {
            console.log('bbbbb',this.isShow)
          this.$emit('hideEmoji')
        }
      })
      $(window).scroll(() => {
      ...
      userReaction () {
        ...
        if (this.selectedMessage) {
          for (const r of this.selectedMessage.reactions) {
            if (r.user.id === this.$page.props.auth.user.id) {
              emojiId = r.emoji_id
              reactionId = r.id
              break
            }
